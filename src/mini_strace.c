...
static void run_tracer(pid_t child, int follow_forks, int show_timing, int summary_mode,
    ...
    for (;;) {
        ...
        if (!ts->in_syscall) {
            ...
            unsigned char getdents_buf_mask = getdents_buf_arg_mask(name);
            int getdents_buf_idx = -1;
            for (int i = 0; i < 6; i++) {
                if (getdents_buf_mask & (1 << i)) {
                    getdents_buf_idx = i;
                    break;
                }
            }
            ...
            unsigned char timespec_out_mask = timespec_out_arg_mask(name);
            int timespec_out_idx = -1;
            for (int i = 0; i < 6; i++) {
                if (timespec_out_mask & (1 << i)) {
                    timespec_out_idx = i;
                    break;
                }
            }
            ...
            unsigned char epoll_events_mask = epoll_events_arg_mask(name);
            int epoll_events_idx = -1;
            for (int i = 0; i < 6; i++) {
                if (epoll_events_mask & (1 << i)) {
                    epoll_events_idx = i;
                    break;
                }
            }
            ...
            if (ts->suppressed) {
                 ...
                 * rt_sigaction's oldact, epoll_wait, and/or a
                 ...
                 * array is unpopulated until the syscall actually
                ...
                if (!summary_mode) {
                    ...
                    unsigned char epoll_op_mask = epoll_op_arg_mask(name);
                    ...
                    unsigned char timespec_in_mask = timespec_in_arg_mask(name);
                    ...
                    for (int i = 0; i < 6; i++) {
                        ...
                        else if (epoll_op_mask & (1 << i))
                            format_epoll_op(raw_args[i], argbuf[i], sizeof(argbuf[i]));
                        else if (epoll_event_mask & (1 << i))
                            format_epoll_event(wpid, raw_args[i], argbuf[i], sizeof(argbuf[i]));
                        ...
                        else if (timespec_in_mask & (1 << i))
                            format_timespec(wpid, raw_args[i], argbuf[i], sizeof(argbuf[i]));
            ...
            if (ts->suppressed) {
                ...
                if (ts->pending_read_entry != NULL || ts->pending_sockaddr_entry != NULL ||
                     ...
                     * kernel filled in) is available. Any of the ten
                     ...
                     * timespec, wait4's rusage, or epoll_wait's output
                     * array) or combined (recvfrom: a deferred data
                     * buffer *and* a deferred sockaddr; wait4: a
                     * deferred wstatus *and* a deferred rusage, in the
                     * same call) —
                    ...
                    if (!summary_mode) {
                        ...
                        /* Same story for nanosleep/clock_nanosleep's
                         * requested-duration timespec: it's populated
                         * at entry, but the call is routed through the
                         * deferred path unconditionally because of its
                         * *other* (kernel-populated) timespec
                         * argument. clock_settime has no deferred
                         * argument, so it never reaches here — its
                         * timespec is decoded at the entry-stop
                         * instead, further up. */
                        unsigned char timespec_in_mask = timespec_in_arg_mask(ts->pending_name);
                        /* clock_gettime and clock_nanosleep's clockid
                         * argument is entry-populated too, but both
                         * syscalls are always routed through the
                         * deferred path (clock_gettime's only argument
                         * besides clockid is its kernel-populated
                         * result; clock_nanosleep always has a
                         * deferred remaining-time argument), so like
                         * timespec_in_mask above, clockid needs its own
                         * dispatch here rather than relying on the
