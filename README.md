...
`socket`/`socketpair`'s domain and type are a different shape again —
each is a single enum value, not bits to OR at all (a socket is never
both `AF_INET` and `AF_UNIX` at once), so both are a straight lookup
against a small table instead of a flag walk. Type has one wrinkle
domain doesn't: Linux lets `SOCK_CLOEXEC`/`SOCK_NONBLOCK` ride along
OR'd into the same int as the base type (they're literally
`O_CLOEXEC`/`O_NONBLOCK` reused, chosen because those bits don't
